...
OctreeDisplay::OctreeDisplay() : OctreeBaseDisplay()
{
	...
	for (const OctreeVoxelType& type : { UFOMAP_OCCUPIED, UFOMAP_FREE, UFOMAP_UNKNOWN })
	{
		...
		auto color_it = color_property_.insert(
				type, new rviz::ColorProperty("Color", default_color, "", coloring_it.value(),
																			SLOT(updateOctreeColorMode()), this));
		if ("X-Axis" == default_coloring || "Y-Axis" == default_coloring ||
				"Z-Axis" == default_coloring)
		{
			color_it.value()->hide();
		}
...
void OctreeDisplay::update(float wall_dt, float ros_dt)
{
	if (should_update_)
	{
...
void OctreeDisplay::octreeCallback(const ufomap_msgs::Ufomap::ConstPtr& msg)
{
...
void OctreeDisplay::colorPoint(rviz::PointCloud::Point& point,
...
void OctreeDisplay::updateOctreeColorMode()
{
...
bool OctreeDisplay::checkType(const std::string& type) const
{
...
PLUGINLIB_EXPORT_CLASS(ufomap_rviz_plugin::OctreeDisplay, rviz::Display)